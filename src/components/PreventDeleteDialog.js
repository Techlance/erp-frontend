...
const PreventDeleteDialog = ({
  ...
  const [openNest, setOpenNest] = useState(keys.map(()=>{return(true)}));
    ...
    <>
      ...
      >
        <DialogTitle id="alert-dialog-title">
          <Typography variant="h4" color="error">Deletion Not Permitted</Typography>
        </DialogTitle>
        <DialogContent dividers>
        <DialogContentText>
          Deletion of this record is not permitted due to its existing related records.
        </DialogContentText>
        <DialogContentText>
          To delete this record please delete the below child records first.
        </DialogContentText>
        {keys.map((key,index)=> (<List component="nav" aria-labelledby="nested-list-subheader" className={classes.root}>
            ...
            <Collapse in={openNest[index]} timeout="auto" unmountOnExit>
                <List component="div" disablePadding>
                  {checkList[key].map((sub)=> (<ListItem button className={classes.nested}>
                        <ListItemIcon>
