...
const LedgerBillwise = () => {
  ...
  const makeBillwise = ()=>{
    console.log(values.fc_name)
    console.log(company.base_currency)
    return values.billwise.map(val=>{
      return({
        ref_no:val.ref_no,
        fc_amount:(values.fc_name && values.fc_name?.id!==company.base_currency)?parseInt(val.fc_amount):parseInt(val.amt),
    ...
    <Grid container spacing={gridSpacing} justifyContent="center">
      <Grid item sm={12} md={12}>
          <Grid container spacing={gridSpacing}>
            ...
            <pre>{JSON.stringify(ledger_billwise,null,2)}</pre> */}
