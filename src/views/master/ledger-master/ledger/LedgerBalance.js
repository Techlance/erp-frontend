...
const LedgerBalance = () => {
  ...
  const { ledger_balance } = useSelector(
  ...
  const handleUpdate = async ()=>{
    ...
    let form = {
      ...
      fc_amount:(values.fc_name && values.fc_name?.id!==company.base_currency)?parseInt(values.fc_amount):parseInt(values.amt),
      ...
      <Grid item sm={12} md={8}>
        <SubCard title="Edit Ledger Balance">
          <Grid container spacing={gridSpacing}>
            ...
            <pre>{JSON.stringify(ledger_balance,null,2)}</pre>
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              id="fc_amount"
              label="FC Amount"
              value={(values?.fc_name && values?.fc_name?.id!==company.base_currency)?(values?.fc_amount):0}
              InputLabelProps={{ shrink: true }}
              onChange={handleChange}
              type="number"
              inputProps={{
                min:"0",
                readOnly:!(values?.fc_name && values?.fc_name?.id!==company.base_currency)
              }}
            />
          </Grid>
          <Grid item xs={12} sm={6}>
